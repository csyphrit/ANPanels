...
foreach ($rooms as $room) {
	...
	foreach ($events as $schedule) {
		...
		if ($schedule['day'] == 'Friday') {
			$friday[$room['id']][$schedule['time']] = $event['name'] . ($event['adult'] ? ' (18+)' : '');
		} elseif ($schedule['day'] == 'Saturday') {
			$saturday[$room['id']][$schedule['time']] = $event['name'] . ($event['adult'] ? ' (18+)' : '');
		} elseif ($schedule['day'] == 'Sunday') {
			$sunday[$room['id']][$schedule['time']] = $event['name'] . ($event['adult'] ? ' (18+)' : '');
		}
	}
}
?>
